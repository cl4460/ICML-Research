...
def convert_json_to_doc_data(json_file: str):
    """
    Read a JSON file and convert it into a structured doc_data format.
    ...
    required_fields = ['question', 'ground_truth', 'model_response']
    for field in required_fields:
        if not all(field in entry for entry in data):
            raise ValueError(f"JSON file must contain '{field}' field in all entries.")
    
    # If 'question_type' is not present, set it to 'N/A'
    doc_data = []
    for entry in data:
        doc_data.append({
...
def read_ground_truth(json_file: str):
    ...
    if not all('question' in entry and 'ground_truth' in entry for entry in data):
        raise ValueError("JSON file must contain 'question' and 'ground_truth' fields in all entries.")
    
    # Ensure each query has a unique ground truth
    ground_truth = []
    seen_queries = set()
    for entry in data:
        query = entry['question']
